...
class MonPythagore(Scene):
    ...
    def PythagoreNonRectangle(self):
        ...
        for t in text:
            self.play(Write(t))
        self.play(Transform(tri_rect, orig_tri), run_time=3)
        ...
        self.play(Create(Angle.from_three_points(A, C, B)),
            Write(MathTex(r"\hat A").next_to(C, UR).scale(eq_scale)))
        self.play(Create(Angle.from_three_points(B, C, H, radius=0.5)),
            Write(MathTex(r"\pi - \hat A").next_to(C, UL).scale(eq_scale)))

        txt = Tex(r"Dans le triangle rectangle BHC, on a:")
        eq_y = MathTex(r"{{y}}=a sin(\pi - \hat A) = {{a sin(\hat A)}}")
        eq_z = MathTex(r"z=a cos(\pi - \hat A) = -a cos(\hat A)")
        eq_x = MathTex(r"{{x}} = b + z = {{b - a cos(\hat A)}}")
        eq_or = MathTex(r"{{c^2}} = x^2 + y^2")
        eq = MathTex(r"{{c^2}} = {{x}}^2 + {{y}}^2 = ({{b - a cos(\hat A)}})^2 +{{(asin(\hat A))}}^2").next_to(eq_x, DOWN)
        eq2 = MathTex(r"c^2 = b^2 + a^2 -2abcos(\hat A)")
