...
class cycloides(Scene):
    ...
    def Intro(self):
        texte= VGroup(Text("Prenons un cercle de centre C", t2c={'cercle':RED, 'centre':BLUE}),
                      Text(r"et considérons un point sur le rayon vertical.", t2c ={'point':YELLOW}),
                      Text(r"La trajectoire de ce point quand le cercle se déplace", t2c={'cercle':RED, 'point':YELLOW}),
                      Text(r"s'appelle une cycloïde.", t2c ={'cycloïde.':GREEN})) \
    ...
    def CyclAvecPolygones(self):
        ...
        texte= VGroup(Text("Prenons un polygône régulier à n côtés avec:", t2c={'polygone':BLUE}),
                      Text("son centre", t2c={'centre':GREEN}),
                      Text("son projeté orthogonal sur l'axe", t2c ={'projeté orthogonal':YELLOW}),
                      Text("le sommet à droite sur l'axe", t2c={'sommet':RED})
                      ).scale(0.8).arrange(DOWN).to_edge(UP)
        self.play(*[Write(t) for t in texte], 
        ...
        self.play(LaggedStart(
            FadeOut(Text("On retrouve le cercle circonscrit de chaque polygone.",
                         t2c={'polygone':BLUE, 'cercle':GREEN}).scale(0.5).to_edge(UP)),
            FadeOut(Text("On devrait donc retrouver la cycloide du cercle").scale(0.5).to_edge(DOWN)),
            lag_ratio = 0.85, run_time=5))
